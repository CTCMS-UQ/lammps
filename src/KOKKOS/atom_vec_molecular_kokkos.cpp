...
void AtomVecMolecularKokkos::unpack_comm(int n, int first, double *buf)
{
  ...
  // NOTE: Should this call atomKK->modified()??
}
...
int AtomVecMolecularKokkos::pack_border(int n, int *list, double *buf,
                                        int pbc_flag, int *pbc)
{
  constexpr auto border_mask = X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|MOLECULE_MASK;
  // NOTE: should this call sync?
  int m = 0;
...
int AtomVecMolecularKokkos::pack_border_vel(int n, int *list, double *buf,
                                            int pbc_flag, int *pbc)
{
  constexpr auto border_mask = X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|MOLECULE_MASK|V_MASK;
  // NOTE: should this call sync?
  int m = 0;
...
{
  constexpr auto mod_mask = X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|MOLECULE_MASK;
  int m = 0;
  int last = first + n;
...
{
  constexpr auto mod_mask = X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|MOLECULE_MASK|V_MASK;
  int m = 0;
  int last = first + n;
  while (last > nmax) grow(0);
...
double AtomVecMolecularKokkos::memory_usage()
{
  double bytes = 0;
